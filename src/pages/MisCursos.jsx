...
export default function MisCursos() {
  ...
  const cargar = async () => {
    ...
    const aprobadas = matriculas.filter(m => m.estado === 'aprobado');
    const enPendiente = matriculas.filter(m => m.estado === 'pendiente');

    const cursosData = await Promise.all(
      aprobadas.map(async (m) => {
        ...
        return { ...curso, entregas, notaFinal, totalEntregas: entregas.filter(e => e.estado === 'evaluado').length };
      })
    );

    const pendientesData = await Promise.all(
      enPendiente.map(async (m) => {
        const curso = await getCurso(m.cursoId);
        return curso ? { ...curso, matriculaEstado: 'pendiente' } : null;
      })
    );

    setCursos(cursosData.filter(Boolean));
    setPendientes(pendientesData.filter(Boolean));
    setLoading(false);
  };
  ...
  return (
    <div style={s.container}>
      ...
      {pendientes.length > 0 && (
        <div style={s.pendientesBox}>
          <h3 style={s.pendientesTitle}>⏳ Solicitudes pendientes de aprobación</h3>
          {pendientes.map((c, i) => (
            <div key={i} style={s.pendienteRow}>
              <span style={s.pendienteNombre}>📚 {c.nombre}</span>
              <span style={s.pendienteMeta}>{c.docenteNombre} · {c.ciclo}</span>
              <span style={s.pendienteBadge}>Esperando aprobación del docente</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={s.statsRow}>
        {[
          { icon: '📚', label: 'Cursos activos', value: cursos.length },
          { icon: '📝', label: 'Entregas', value: cursos.reduce((s, c) => s + c.totalEntregas, 0) },
          {
            ...
            value: cursos.filter(c => c.notaFinal !== null).length > 0
              ? (cursos.filter(c => c.notaFinal !== null).reduce((s, c) => s + c.notaFinal, 0) / cursos.filter(c => c.notaFinal !== null).length).toFixed(1) + '/20'
              : '—'
          },
        ].map((st, i) => (
      ...
      <h2 style={s.sectionTitle}>Mis Cursos Activos</h2>
      {cursos.length === 0 ? (
        <div style={s.emptyState}>
          ...
          <button style={s.joinBtn} onClick={() => navigate('/unirse')}>+ Unirme a un curso</button>
        </div>
      ) : (
            ...
            return (
              <div key={i} style={{ ...s.cursoCard, borderTop: `3px solid ${color}` }}
                ...
                <div style={s.cursoFooter}>
                  <div>
                    <p style={s.notaLabel}>Nota actual</p>
                    <p style={{ ...s.notaValue, color }}>{c.notaFinal !== null ? `${c.notaFinal}/20` : '—'}</p>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={s.entregasLabel}>{c.totalEntregas} evaluado{c.totalEntregas !== 1 ? 's' : ''}</p>
                  </div>
                </div>
...
const s = {
  ...
  pendienteNombre: { color: '#fff', fontSize: '14px', fontWeight: '500' },
  pendienteMeta: { color: 'rgba(255,255,255,0.4)', fontSize: '12px' },
  pendienteBadge: { marginLeft: 'auto', color: '#f59e0b', fontSize: '11px', background: 'rgba(245,158,11,0.15)', padding: '3px 10px', borderRadius: '6px' },
  ...
  cursoCard: { background: 'rgba(255,255,255,0.04)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '8px' },
  ...
  cursoNombre: { color: '#fff', fontSize: '17px', fontWeight: '600', margin: 0 },
  cursometa: { color: 'rgba(255,255,255,0.4)', fontSize: '13px', margin: 0 },
  ...
  cursoFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '4px' },
  notaLabel: { color: 'rgba(255,255,255,0.4)', fontSize: '11px', margin: '0 0 2px' },
